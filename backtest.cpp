...
run_backtests(const std::vector<trade_t> &trades,
  ...
  for (auto &backtest : backtests) {
    ...
    auto historic_price_index = backtest.prices.cbegin();
    auto current_price_index = std::next(historic_price_index, analysis_window);
    auto future_price_index = std::next(current_price_index, sell_window);

    // Move windows along until we run out of prices
    while (future_price_index < backtest.prices.cend()) {
      ...
      if (backtest.strategy.execute(historic_price_index,
        ...
        if (const auto sell_price_index =
          ...
          const int buy_index =
              std::distance(backtest.prices.cbegin(), current_price_index);
          const int sell_index =
              std::distance(backtest.prices.cbegin(), sell_price_index);
    ...
    historic_price_index = std::prev(backtest.prices.cend(), analysis_window);
    current_price_index = backtest.prices.cend();

    if (backtest.strategy.execute(historic_price_index, current_price_index))
      backtest.buy = true;
  }
  ...
  //               >
  //                      static_cast<double>(bgd ? bgd : .9) / (bbd ? bbd :
  //                      .9);
  //             });
