...
int main() {
  ...
  std::for_each(
      coins.cbegin(), coins.cend(), [&buys, &sells](const auto &name) {

        if (std::find_if(buys.begin(), buys.end(), [&name](const auto &coin) {
              return coin.name == name;
            }) != buys.cend())
          std::cout << name << " prepare to buy\n";

        if (std::find_if(sells.begin(), sells.end(), [&name](const auto &coin) {
              return coin.name == name;
            }) != sells.cend())
